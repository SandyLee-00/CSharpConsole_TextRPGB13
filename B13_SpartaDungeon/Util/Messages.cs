namespace B13_SpartaDungeon.Util;

public static class AlertMessages
{
    public const string BLANK_LINE_COVER = "             ";

    public const string ERROR_INPUT = "잘못된 입력입니다.";
    // public const string COMPLETE_PURCHASE = "\n구매를 완료했습니다.";
    // public const string LACKED_GOLD = "\nGold 가 부족합니다.";
    // public const string PURCHASED = "\n이미 구매한 아이템입니다.";
    // public const string IMPOSSIBLE_SELL = "\n판매할 수 없습니다.";
}

public static class ChoiceMessage
{
    public const string BASIC = "원하시는 행동을 입력해주세요.";
    public const string TARGET = "대상을 선택해주세요.";
}